...
(function ( $ ) {
	...
	FlatSliderObj = {
		...
		_create: function () {
            ...
            // Set the value to current element with givin initiall value.
		...
		refresh: function () {
		   // debugger;
		    var $handle = this.$slider.find('.ui-slider-handle');
		    var ctrl = this.element.val();
		    ctrl = $.trim(ctrl);
		    this.$wert.html(ctrl + ' ' + this.options.einheit);
		    $handle.css("left", ctrl + "%");
		    this._update_normal_handle($handle);
		},
		...
		on_slide: function (event, ui) {
			...
			if (typeof (this.options.slide) == "function") {
			    if (this.options.range === true) {
			        this.options.slide.call(this, ui.values[0]);
			    }
			    else {
			        this.options.slide.call(this, ui.value);
			    }
			}
		},
